...
void main() {
	...
	string folder = "E:\\Sample1";		//change to the abosolute path where sample1 locates
	...
	for (int i = 1; i < (sliceNumber+1); i++) {
		string s;
		stringstream ss;
		ss << i;
		ss >> s;
		imagePath.push_back(folder + "\\segmented\\segmented (" + s + ").png");
		originPath.push_back(folder + "\\origin\\origin (" + s + ").png");
	}
